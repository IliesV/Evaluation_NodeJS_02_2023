...
//Création et export d'une méthode qui permet de récupérer toute les données contenu dans le tableau associé au controller (ici "metallica")
exports.getAllData = (request, response) => {
    ...
    fs.readFile("./src/models/albums.json", (err, data)=>{
        ...
        //Sinon on renvoie un code 200.
            ...
            response.status(200).json(JSON.parse(data).metallica);
        }
    })
}
...
exports.getDataById = (request, response) =>{
    ...
    fs.readFile("./src/models/albums.json", (err,data)=>{
        ...
        if (err) {
            ...
            const dataById = existingData.metallica.find((obj) => obj.id === parseInt(request.params.id));
...
exports.getDataByTitle = (request, response)=> {
    ...
    fs.readFile("./src/models/albums.json", (err, data)=>{
        ...
        if (err){
            ...
            const dataByTitle = existingData.metallica.find((obj)=> obj.titre === request.params.titre)
...
exports.createData = (request, response) =>{
    ...
    fs.readFile("./src/models/albums.json", (err, data)=>{
        ...
        //Sinon on stock notre jeu de données dans une constante
            ...
            if (existingData.metallica === []) {
                existingData.metallica.push({ "id": 1, "titre": request.body.titre, "année": request.body.année });
            //Sinon on ajoute les paramètres fournis par l'utilisateur et on ajoute un id correspondant à la taille du tableau + 1
            } else {
                existingData.metallica.push({ "id": existingData.metallica.length+1, "titre": request.body.titre, "année": request.body.année });
            }
    ...
    fs.readFile("./src/models/albums.json", (err, data)=>{
        ...
        //Sinon on stock notre jeu de données dans une constante
            ...
            //Sinon on supprime l'objet correspondant grace à la méthode filter qui ne garder dans le tableau que les objets dont l'ID ne correspond pas à celui fourni par l'utilisateur
                ...
                existingData.metallica = existingData.metallica.filter((obj)=> obj.id != parseInt(request.params.id));
